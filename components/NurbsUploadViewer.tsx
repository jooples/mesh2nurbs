...
export function validateNurbsData(json: unknown): NurbsData {
...
export default function NurbsUploadViewer({
  initialUrl,
  initialLabel = "Sample surface",
  emptyHint = "Upload or drop a NURBS JSON file to preview it here.",
  className,
}: NurbsUploadViewerProps) {
  const [data, setData] = useState<NurbsData | null>(null);
  const [source, setSource] = useState<string>(initialLabel);
  ...
  const fetchInitial = useCallback(() => {
    ...
    fetch(initialUrl)
      ...
      .then((json) => {
        ...
        setSource(initialLabel);
      })
      .catch(() => setError("Failed to load the NURBS surface."))
